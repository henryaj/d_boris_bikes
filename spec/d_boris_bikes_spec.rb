...
describe Bike do
	...
	context "writing to bikes.csv" do 
		def wipe_csv
		...
		it "writes all attributes for a bike to a CSV row" do 
			wipe_csv
			bike1 = Bike.new
			expect(csv_file.last).to eq [bike1.serial,"#{bike1.broken?}","#{bike1.rented?}","#{bike1.location}"]
		end
		...
		it "should write is location to the cav file" do 
			wipe_csv
			bike1 = Bike.new
			expect(csv_file.flatten).to include("detached")
		end

		xit "once returned, it is in a station" do
			bike.rent!
			bike.return_to(station)
			expect(bike.location)
		end

		xit "should update its location when it is returned to a dock" do
			bike.rent!
			bike.return_to(station)
		end
